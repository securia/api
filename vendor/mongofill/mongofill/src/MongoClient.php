...
class MongoClient
{
     ...
     * @var array
     */
    private $replSetStatus = array();

    /**
     * @var array
     */
    private $replSetConf = array();
    ...
    private function getNearestHostProtocol(array $allowedServerTypes, array $readPreference)
    {
        ...
        foreach ($tagsets as $tagset) {
            foreach ($this->replSetStatus['members'] as $key => $member) {
                $tags = isset($this->replSetConf['members'][$key]['tags']) ? $this->replSetConf['members'][$key]['tags'] : array();
        ...
        if ($tags) {
            ...
            foreach ($tags as $i => $tagset) {
                if (is_string($tagset)) {
                    ...
                    $tagset = $tagset ? explode(',', $tagset) : array();
