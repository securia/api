...
class CookieTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertEquals($cookie, (string) \Illuminate\Support\Facades\Cookie::fromString($cookie, $url));
    }
    ...
    {
        $this->assertFalse(\Illuminate\Support\Facades\Cookie::fromString('foo=bar; secure')->isSecure());
        $this->assertFalse(\Illuminate\Support\Facades\Cookie::fromString('foo=bar; secure', 'http://example.com/')->isSecure());
    }
    ...
    {
        $this->assertEquals(1596185377, \Illuminate\Support\Facades\Cookie::fromString($cookie)->getExpiresTime());
    }
    ...
    {
        $this->assertEquals('Foo=Bar; path=/', (string) \Illuminate\Support\Facades\Cookie::fromString('Foo=Bar'));
        $this->assertEquals('foo=bar; expires=Fri, 31 Dec 2010 23:59:59 GMT; path=/', (string) \Illuminate\Support\Facades\Cookie::fromString('foo=bar; Expires=Fri, 31 Dec 2010 23:59:59 GMT'));
        $this->assertEquals('foo=bar; domain=www.example.org; path=/; httponly', (string) \Illuminate\Support\Facades\Cookie::fromString('foo=bar; DOMAIN=www.example.org; HttpOnly'));
    }

    public function testFromStringWithUrl()
    {
        $this->assertEquals('foo=bar; domain=www.example.com; path=/', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar', 'http://www.example.com/'));
        $this->assertEquals('foo=bar; domain=www.example.com; path=/', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar', 'http://www.example.com'));
        $this->assertEquals('foo=bar; domain=www.example.com; path=/', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar', 'http://www.example.com?foo'));
        $this->assertEquals('foo=bar; domain=www.example.com; path=/foo', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar', 'http://www.example.com/foo/bar'));
        $this->assertEquals('foo=bar; domain=www.example.com; path=/', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar; path=/', 'http://www.example.com/foo/bar'));
        $this->assertEquals('foo=bar; domain=www.myotherexample.com; path=/', (string) \Illuminate\Support\Facades\Cookie::FromString('foo=bar; domain=www.myotherexample.com', 'http://www.example.com/'));
    }

    public function testFromStringThrowsAnExceptionIfCookieIsNotValid()
    {
        $this->setExpectedException('InvalidArgumentException');
        \Illuminate\Support\Facades\Cookie::FromString('foo');
    }

    public function testFromStringThrowsAnExceptionIfCookieDateIsNotValid()
    {
        $this->setExpectedException('InvalidArgumentException');
        \Illuminate\Support\Facades\Cookie::FromString('foo=bar; expires=Flursday July 31st 2020, 08:49:37 GMT');
    }

    public function testFromStringThrowsAnExceptionIfUrlIsNotValid()
    {
        $this->setExpectedException('InvalidArgumentException');
        \Illuminate\Support\Facades\Cookie::FromString('foo=bar', 'foobar');
    }
